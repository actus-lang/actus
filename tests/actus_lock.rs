...
use actus::configuration::{ActusLock, LockedPackage};
...
impl Fixture {
    ...
    fn manifest(&self, dependencies: &str) -> PathBuf {
        let path = self.root.join("Actus.toml");
        fs::write(
            &path,
            format!(
...
fn serializes_packages_in_deterministic_order() {
    let lock = ActusLock::from_packages(vec![
        LockedPackage {
            name: "zeta".to_owned(),
...
fn generates_and_validates_lockfile_against_manifest_dependencies() {
    ...
    let path = ActusLock::sync(&manifest).expect("lockfile should be written");
    assert_eq!(path, fixture.root.join("Actus.lock"));
    let source = fs::read_to_string(path).expect("read generated lockfile");
    assert_eq!(ActusLock::parse(&source).expect("parse generated lockfile"), lock);
}
...
fn rejects_lockfile_that_does_not_match_manifest() {
    ...
    let stale = ActusLock::from_packages(vec![LockedPackage {
        name: "alpha".to_owned(),
        version: "1.0.0".to_owned(),
        source: Some("registry".to_owned()),
        path: None,
